  ...
  render() {
    ...
    this.state.data.forEach(item => {
      if(i<3) { // just showing 3 items. you should control this in the View instead
        var summary = item.body[0].summary.substring(0,110); // truncating summary field
        var path = "/node/" + item.nid[0].value; // creating path to node
        blob.push(
     ...
     return (
        <div className="view view--articles--alpha">
          <div className="view-header">
            <h2 className="title">Articles: Recently Authored (1s)</h2>
          </div>
...
ReactDOM.render(
  ...
  <App source="http://localhost:8888/api/articles/recently-authored" />,
  document.getElementById('react--view--articles--recently-authored')
